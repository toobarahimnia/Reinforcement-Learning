...
class ReplayBuffer(object):
    def __init__(self, max_size, input_shape, n_actions):
        ...
        self.mem_cntr = 0
        self.state_memory = np.zeros((self.mem_size, *input_shape)) # what does * mean? https://scripteverything.com/what-does-asterisk-before-variable-name-mean-in-python/
...
class Actor(object):
    def __init__(self, lr, n_actions, name, input_dims, sess, fc1_dims,
        ...
        self.checkpoint_file = os.path.join(chkpt_dir, name+'_ddpg.chpt')
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.action_gradient = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_actions], name='gradients')

            f1 = 1 / np.sqrt(self.fc1_dims)
            dense1 = tf.compat.v1.layers.dense(self.input, units=self.fc1_dims,
                            kernel_initializer = tf.random_uniform_initializer(-f1, f1, seed=None),
                            bias_initializer = tf.random_uniform_initializer(-f1, f1, seed=None))
            ...
            dense2 = tf.compat.v1.layers.dense(layer1_activation, units=self.fc2_dims,
                            kernel_initializer = tf.random_uniform_initializer(-f2, f2, seed=None),
                            bias_initializer = tf.random_uniform_initializer(-f2, f2, seed=None))
            ...
            mu = tf.compat.v1.layers.dense(layer2_activation, units=self.n_actions,
                                    activation='tanh',
                                    kernel_initializer=tf.random_uniform_initializer(-f3, f3, seed=None),
                                    bias_initializer=tf.random_uniform_initializer(-f3, f3, seed=None))
...
class Critic(object):
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            action_in = tf.compat.v1.layers.dense(self.actions, units=self.fc2_dims, activation='relu')
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
        ...
        self.batch_size = batch_size
        # self.batch_size = np.reshape(self.batch_size, (-1, 1))
