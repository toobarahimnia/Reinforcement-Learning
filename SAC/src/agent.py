...
class Agent:
    def __init__(self, env, path_save=PATH_SAVE, path_load=PATH_LOAD, actor_lr=ACTOR_LR, critic_lr=CRITIC_LR, gamma=GAMMA, tau=TAU, reward_scale=REWARD_SCALE):
        ...
        self.critic_0.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.critic_lr))
        self.critic_1.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.critic_lr))
        self.critic_value.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.critic_lr))
        self.critic_target_value.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.critic_lr))
